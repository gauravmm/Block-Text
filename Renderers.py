...
class ShellRenderer(AbstractRenderer):
	def render(self, layoutWrapper):
		...
		for ln in lines:
			...
			print()
			print("".join(self.glyphTransform(c) for c in [True, False] * (layoutWrapper.getLineWidth()//2)))
			print()
...
class SplitRenderer(AbstractRenderer):
	def __init__(self, bufCount, density):
		...
		self.numTrueInDist = lambda bC, median, val: round(random.triangular(1, bC, median)) if val else 0
	...
	def render(self, layoutWrapper):
		lines = layoutWrapper.get()
		outBufs = [[[self.generateDist(self.bufCount, self.median, r) for r in col] for col in ln] for ln in lines]
		# Transpose outBufs from [lines][columns][cells][buffer_number] to [buffer_number][lines][columns][cells]:
		return [ReadOnlyLayoutWrapper(layoutWrapper, [[[r[bidx] for r in col] for col in ln] for ln in outBufs]) for bidx in range(self.bufCount)]
